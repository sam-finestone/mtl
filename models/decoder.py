...
class SegDecoder(nn.Module):
    def __init__(self, input_dim, num_classes, drop_out, BatchNorm):
        ...
        self.conv1 = nn.Conv2d(input_dim, 48, 1, bias=False)
    ...
    def forward(self, x, low_level_feat):
        ...
        x = F.upsample(x, size=(128, 256), mode='bilinear', align_corners=True)
        x = self.last_conv(x)
        return x
...
class SegDecoder2(nn.Module):
    def __init__(self, input_dim, num_classes, drop_out, BatchNorm):
        super(SegDecoder2, self).__init__()
        self.input_height = input_dim
        # adopt [1x1, 48] for channel reduction.
        # self.conv1 = nn.Conv2d(input_dim, 48, 1, bias=False)
        # self.bn1 = BatchNorm(48)
...
class DecoderTemporal(nn.Module):
    def __init__(self, input_c_dim, num_classes, drop_out, BatchNorm):
        ...
        # self.bn1 = BatchNorm(48)
        # self.relu = nn.ReLU()
        self.last_conv = nn.Sequential(nn.Conv3d(input_c_dim, 256, kernel_size=(3, 3, 3), stride=1, padding=1, bias=False),
                                       BatchNorm(256),
                                       nn.ReLU(),
                                       nn.Dropout(drop_out),
                                       nn.Conv3d(256, 256, kernel_size=(3, 3, 3), stride=1, padding=1, bias=False),
                                       BatchNorm(256),
                                       nn.ReLU(),
                                       nn.Dropout(drop_out),
                                       nn.Conv3d(256, num_classes, kernel_size=(1, 1, 1), stride=1))
        self._init_weight()
    ...
    #     low_level_feat = low_level_feat.permute(0, 2, 1, 3, 4)
