...
from models.decoder import Decoder, SegDecoder, DepthDecoder, MutliDecoder
...
from utils.static_helpers import *
...
if TASK == 'segmentation':
    ...
    dec = MutliDecoder(image_size, CLASS_TASKS, drop_out, SynchronizedBatchNorm1d).to(device)
...
with open(os.path.join(LOG_FILE, 'log_epoch.txt'), 'a') as epoch_log:
    epoch_log.write('epoch, train loss, train segmentation acc, train depth error, '
                    'train loss, val segmentation acc, val depth error miou \n')

with open(os.path.join(LOG_FILE, 'log_train_batch.txt'), 'a') as f:
    f.write('epoch, batch index, train loss, train segmentation acc, train depth error \n')
