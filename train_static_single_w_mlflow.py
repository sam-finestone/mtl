...
with mlflow.start_run():
    ...
    for epoch in range(EPOCHS):
        ...
        if TASK == 'segmentation':
            ...
            # Validate
            print('--- Validation ---')
            val_acc, val_loss, miou = static_test_single_task(epoch, criterion, val_dataloader,
                                                              model, TASK, classLabels,
                                                              validClasses, SAMPLES_PATH, void=0,
                                                              maskColors=None, args=args)
    ...
    if TASK == 'segmentation':
        metrics_test = {'test_loss': [],
                        ...
                        'test_miou': []}
        # test set
        test_acc, test_loss, miou = static_test_single_task(1, criterion, test_dataloader,
        ...
        metrics_test['test_miou'].append(miou)
        print('Epoch {} test loss: {:.4f}, acc: {:.4f}, miou: {:.4f}'.format(epoch, test_loss, test_acc, miou))

    elif TASK == 'depth':
        metrics_test = {'test_loss': [],
                        'test_abs_error': [],
                        'test_rel_error': []}
        test_rel_error, test_abs_error, test_loss = static_test_single_task(1, criterion, test_dataloader,
                                                                            loaded_model, TASK, classLabels,
                                                                            validClasses, SAMPLES_PATH, void=0,
                                                                            maskColors=None, args=None)
